...
- `outputs/`: 每次任务的输出结果
- `archives/`: 历史高表现脚本与复盘
- `app/` + `components/` + `lib/`: Next.js Web 控制台

## 项目架构 Architecture

系统当前是一个“知识资产 + CLI 流水线 + Next.js 控制台”结构：

1. `products/` + `knowledge/`
   承载长期知识，包括产品资料、用户洞察、竞手分析、风格约束。
2. `inputs/` + `tasks/`
   承载单次任务输入，包括脚本参数、任务 JSON、批量任务 JSON。
3. `run.js`
   核心执行器，负责：
   - 读取产品与任务输入
   - 调用 DeepSeek
   - 执行 `generate -> review -> revise`
   - 写入输出
   - 更新索引
   - 可选归档高表现脚本
4. `report.js`
   基于 `outputs/index.json` 和 `archives/high_performers/index.json` 生成复盘报告。
5. `app/` + `components/` + `lib/`
   Next.js 工作台，负责产品资料编辑、任务发起、结果审稿、表现回填、报告预览。
6. `outputs/` + `archives/high_performers/` + `reports/`
   分别负责日常输出、优秀资产沉淀、复盘汇总。

Web 版没有改动核心链路，仍然直接复用 `run.js` / `report.js`。
...
- 创建产品目录
- 通过表单维护 `product_brief` / `audience_insight`
- 人工维护 processed 竞手分析
- 发起单任务
- 执行批量任务
- 直接粘贴竞手原稿并预处理
...
## 竞手原稿预处理 Competitor Raw Preprocess

如果你手里拿到的是竞手视频的原始口播稿，而不是已经整理好的分析卡，可以先跑预处理命令。
...
然后在正式任务里，把 `competitor` 指向这份处理后的文件即可。

`inputs/competitor_processed_template.md` 默认不是日常填写入口，它主要是结构参考和人工兜底模板。
